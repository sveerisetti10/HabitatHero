...
from .rag_model import generate_chat_response
...
def chat():
    ...
    # We can define the role of the user and the content of the message
    session['conversation'].append({'role': 'user', 'content': message})
    # The generate chat response function is defined in the rag_model.py file
    response_message = generate_chat_response(session['conversation'])
    session['conversation'].append({'role': 'assistant', 'content': response_message})
    # Here we return the response message
